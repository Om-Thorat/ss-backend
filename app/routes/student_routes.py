...
from app.routes.login_routes import verify
...
def get_student_details():
    ...
    try:
        with db.engine.connect() as conn:
            ...
            student_query = text(
                'SELECT * FROM student WHERE student."RollNo" = :rollno'
            )
            ...
            club_members_result = conn.execute(
                club_members_query, {"rollno": rollno}
            ).fetchall()
            ...
            coordinator_clubs_result = conn.execute(
                coordinator_clubs_query, {"rollno": rollno}
            ).fetchall()
            ...
            cr_courses_result = conn.execute(
                cr_courses_query, {"rollno": rollno}
            ).fetchall()
...
def update_student():
    ...
    if not verify(data.get("rollno"), token):
    ...
    if hometown:
        ...
        params["hometown"] = f"{hometown}"
    if blood_group:
        query += ', "BloodGroup" = :blood_group'
        params["blood_group"] = f"{blood_group}"
    if hostel_name:
        query += ', "HostelName" = :hostel_name'
        params["hostel_name"] = f"{hostel_name}"
    ...
    with db.engine.connect() as conn:
        result = conn.execute(text(query), params)
        conn.commit()
        print(result.rowcount, result.context.statement)
        if result.rowcount == 0:
            return jsonify({"error": "Student not found"}), 404

    return jsonify({"message": "Student updated successfully", "RollNo": rollno}), 200
